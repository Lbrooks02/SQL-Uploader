...
def Log_Import(tableName: str, recordCount: int, elapsedTime: str, uploadStartTime: datetime, uploadEndTime: datetime, sourceFile: str): #Export execution data to excel
    Validate_Data("tableName", "recordCount", "elapsedTime", "uploadStartTime", "uploadEndTime", "sourceFile")
    clientName = "TEST"
    workBookPath = os.path.join(os.path.expanduser("~"), "Documents", "SQL_UploadLog.xlsx")  #Import Data is logged here
    ...
    if File_Exists(workBookPath, False):                       #Open workbook and go to Metro Sheet
        createFile = False
        workBook = load_workbook(workBookPath)
        if clientName not in workBook.sheetnames:
            workBook.create_sheet(clientName)
            workBook[clientName].append(headers)
        workSheet = workBook[clientName]    
    else:                                               #Create new workbook if one doesn't exist
        createFile = True
        workBook = Workbook()
        workSheet = workBook.active
        workSheet.title = clientName
    ...
    while True:                                     #Error handler if workbook is already open, loops this until closed
        try:
            ...
            Play_Sound("Error")
            print(e)
            response = input(f">> Import Log is currently open. Close file at: '{workBookPath}' then hit the enter key to continue >> ")     
            if response.upper() == 'STOP':
                sys.exit(">> Cancel input command received.")
    print (f">> Upload logged at '{workBookPath}'")

def Play_Sound(soundType: str):                     #Plays sound based on process success/failure
...
def SQL_Upload(sourceFile: str, serverName: str, databaseName: str, tableName: str, datasetName: str = "", skipRows: list = None, skipColumns: list = None, convertColumnTypes: dict = None, delimiterCharacter: str = '|', encodingType: str = 'latin1'): #Send data to SQL
    """
        Parameters:
            :sourceFile: Str to store source file path
            :tableName: Str to store SQL destination table name 
            :datasetName: *Optional* Str to store datasheet/table name (Excel/Access)
            :skipRows: *Optional* List containing row indexes to skip
            :skipColumns: *Optional* List containing column indexes to skip
            ...
            :delimiterCharacter: *Optional* Str to store delimiter character (for .txt and .csv files)
            :encodingType: *Optional* Str to store encoding style for special characters ("latin1" by default, can also use "utf8")
    ...
    try:
        try:
            ...
            if isExcelFile: 
                if specifyDatasetName:                                #If a sheet name is specified
                    ...
                    excelObject = pd.ExcelFile(sourceFile)
                    sheetList = excelObject.sheet_names
                    if datasetName in sheetList:
                        try:
                            df = pd.read_excel(sourceFile, sheet_name = datasetName, skiprows = [row for row in skipRows])  #Load excel into dataframe
                        except IndexError:
                    ...
                    try:                                                    
                        df = pd.read_excel(sourceFile, skiprows = [row for row in skipRows])                          #Default to first sheet in workbook
                    except IndexError:
                        sys.exit(f">> One or more arguments supplied are out of range for file: {sourceFile}")
                        ...
                        try:
                            if columnName == rsColumn:
                                ...
                                match convertDatatype:
                                    case 'datetime':                 #Format column as datetime (might need to remove the excel condition, if date columns are auto-read as int64 in .txt files )
                                        if (isExcelFile) and ((df[rsColumn].dtypes == 'int64') or (df[rsColumn].dtypes == 'Int64')): 
                                            df[rsColumn] = pd.to_datetime('1899-12-30') + pd.to_timedelta(df[rsColumn], 'D') #Dates in excel are read as integers
                                        elif (isExcelFile) and (df[rsColumn].dtypes == 'float64'): 
                                            ...
                                            df[rsColumn] = pd.to_datetime(df[rsColumn], format="%d.%m.%Y").astype('datetime64[ns]')
        ...
        Log_Import(tableName, recordCount, elapsedTime[0], uploadStartTime, uploadEndTime, sourceFile)  #Append upload session data to log file
        Play_Sound("Success")
        print(f"***** [{tableName}] upload completed in: {elapsedTime[1]}*****", end = "\n\n")
    except KeyboardInterrupt:       #User interrupts the program with Ctrl+C
        sys.exit(">> Cancel input command received.")
